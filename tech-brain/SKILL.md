...
Follow this strict procedure to review and process files from Google Drive into the `TECH-SECOND-BRAIN` directory.
...
Scan ONLY the `00_inbox` folder (located inside `TECH-SECOND-BRAIN`) for files that need processing. Ignore files that are already inside organized project structures unless explicitly told otherwise.
...
- Save any newly created templates in the template folder located at: `'My Drive > TECH-SECOND-BRAIN > .resources'`.

## 4. Folder Maintenance & File Placement
Determine the appropriate `TECH-SECOND-BRAIN` sub-directory (e.g., `01_projects`, `02_areas`, `03_resources`) for the file based on its content and topics.
...
- **Tags:** Retrieve the current `tags.json` file located at `'My Drive > TECH-SECOND-BRAIN > .resources'` and assign the appropriate tag(s).
